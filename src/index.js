...
const init = (args) => {
  const muralUsername = args?.app?.me?.username
  const muralWorkspace = args?.app?.currentTeam?.username
  const muralCompany = args?.app?.currentTeam?.organization
  console.log('muralUsername => ', muralUsername)

  userApi.getOrCreateMuralUser({ muralUsername, muralWorkspace, muralCompany }).then(() => {
